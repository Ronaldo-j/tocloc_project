...
def criar_quadra(nomeQuadra, preco, localizacao, descricao, foto):
    try:
        conexao = criar_conexao()
        cursor = conexao.cursor()
        cursor.execute("""
            INSERT INTO quadra (nomeQuadra, preco, localizacao, descricao, foto)
            VALUES (%s, %s, %s, %s, %s)
        """, (nomeQuadra, preco, localizacao, descricao, foto))
        conexao.commit()
        return {"message": "Quadra cadastrada com sucesso"}, 201
