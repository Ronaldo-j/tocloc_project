...
def criar_user(nomeUsuario, email, password):
    try:
        ...
        # Verificar se a senha já foi utilizada por outro usuário
        cursor.execute("SELECT COUNT(*) FROM user WHERE password = %s", (password,))
        if cursor.fetchone()[0] > 0:
            return {"error": "Esta senha já foi utilizada por outro usuário."}, 400

        # Inserir novo usuário com a senha em texto simples
        cursor.execute("""
            INSERT INTO user (nomeUsuario, email, password)
            VALUES (%s, %s, %s)
        """, (nomeUsuario, email, password))  # Aqui estamos usando a senha em texto simples
        conexao.commit()
